...
def build_graph():
    ...
    try:
        ...
        # 只使用 Obsidian 模式（ZEP 已禁用）
        errors = []
        if not Config.OBSIDIAN_VAULT_PATH:
            errors.append("请配置 OBSIDIAN_VAULT_PATH")
        ...
        def build_task():
            ...
            try:
                ...
                # ========== Obsidian 模式 ==========
                from ..services.obsidian_graph_builder import ObsidianGraphBuilder

                task_manager.update_task(
                    task_id,
                    status=TaskStatus.PROCESSING,
                    message="初始化 Obsidian 图谱构建服务..."
                )
                ...
                def obsidian_progress_callback(msg, progress_ratio):
                    progress = 20 + int(progress_ratio * 70)
                    task_manager.update_task(
                        task_id,
                        message=msg,
                        progress=progress
                ...
                builder.add_text_batches(
                    ...
                    progress_callback=obsidian_progress_callback
                )

                # 获取图谱信息
                graph_info = builder.get_graph_data()
                ...
                task_manager.update_task(
                    ...
                    result={
                        ...
                        "entity_types": graph_info.get("entity_types", []),
                        "vault_path": builder.vault_path
                    }
                )
                ...
                build_logger.info(f"[{task_id}] Obsidian 图谱构建完成!")
